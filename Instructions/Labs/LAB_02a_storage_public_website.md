...
1. 何かが削除された場合に、論理的な削除を使用してファイルを復元する演習をする必要があります。
    ...
    - **[削除]** を選択して、ファイルの削除を確認します。  
    - コンテナーの**概要**ページで **[論理的な削除]** ドロップダウンを使用し、**[アクティブな、および削除された BLOB を表示する]** を選択します。 このコントロールは、検索ボックスの右側にあります。 
...
1. 時間があれば、以前の BLOB バージョンの復元を試してください。
   ...
   - BLOB を開いて、**[バージョン]** タブを選択すると、前のファイル バージョンを表示できます。 
...
+ [Azure Blob Storage を調べる](https://learn.microsoft.com/training/modules/explore-azure-blob-storage/)。 このモジュールでは、Azure Blob Storage の主要な特徴と機能について説明しています。
